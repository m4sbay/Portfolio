    ...
    <div className="flex flex-col gap-24 lg:gap-32 pb-24">
      ...
      <div className="grid grid-cols-1 items-start gap-12 lg:grid-cols-[1fr_1.5fr] lg:gap-20">
        ...
        <div className="sticky top-24 space-y-8">
          <div>
            <Link href="/" className="group mb-8 inline-flex items-center gap-2 text-sm text-zinc-600 hover:text-zinc-950 dark:text-zinc-400 dark:hover:text-zinc-50">
              <svg
                width="12"
                height="12"
                viewBox="0 0 24 24"
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <h2 className="text-sm font-medium text-zinc-500 dark:text-zinc-400">{project.title}</h2>
                <h1 className="text-2xl font-medium tracking-tight text-zinc-900 dark:text-zinc-50 sm:text-3xl lg:text-4xl lg:leading-tight">{project.description}</h1>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-6 lg:space-y-8">
          {project.gallery && project.gallery.length > 0 ? (
            project.gallery.map((img, idx) => (
              <div key={idx} className="overflow-hidden rounded-2xl border border-zinc-200 bg-zinc-50 dark:border-white/10 dark:bg-white/5">
                <Image src={img.src} alt={img.alt} width={img.width} height={img.height} className="h-auto w-full object-cover" sizes="(min-width: 1024px) 60vw, 100vw" priority={idx === 0} />
              </div>
            ))
          ) : (
            ...
            <>
              <div className="overflow-hidden rounded-2xl border border-zinc-200 bg-zinc-50 dark:border-white/10 dark:bg-white/5">
                <Image src={project.image.src} alt={project.image.alt} width={project.image.width} height={project.image.height} className="h-auto w-full object-cover" sizes="(min-width: 1024px) 60vw, 100vw" priority />
              </div>
              <div className="overflow-hidden rounded-2xl border border-zinc-200 bg-zinc-50 dark:border-white/10 dark:bg-white/5">
                <Image src={project.hoverImage.src} alt={project.hoverImage.alt} width={project.hoverImage.width} height={project.hoverImage.height} className="h-auto w-full object-cover" sizes="(min-width: 1024px) 60vw, 100vw" />
              </div>
            </>
          )}
        </div>
      ...
      {project.caseStudy && (
        <div id="case-study" className="grid grid-cols-1 items-start gap-12 lg:grid-cols-[1fr_1.5fr] lg:gap-20 pt-8 border-t border-zinc-200 dark:border-white/10">
          ...
          <div className="sticky top-24 space-y-8">
            <div className="space-y-4">
              <div className="space-y-2">
                <h2 className="text-sm font-medium text-zinc-500 dark:text-zinc-400">Case Study</h2>
                <h3 className="text-2xl font-medium tracking-tight text-zinc-900 dark:text-zinc-50 sm:text-3xl lg:text-4xl lg:leading-tight">{project.caseStudy.title}</h3>
              </div>
            </div>
          ...
          <div className="space-y-6 lg:space-y-8">
            {project.caseStudy.gallery.map((img, idx) => (
              <div key={idx} className="overflow-hidden rounded-2xl border border-zinc-200 bg-zinc-50 dark:border-white/10 dark:bg-white/5">
                <Image src={img.src} alt={img.alt} width={img.width} height={img.height} className="h-auto w-full object-cover" sizes="(min-width: 1024px) 60vw, 100vw" />
              </div>
            ))}
          </div>
        </div>
