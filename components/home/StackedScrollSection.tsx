...
const visualWrapClass =
  "relative h-44 w-full overflow-hidden bg-zinc-100 sm:h-48 dark:bg-zinc-900/40";

const cardShellClass =
  "flex flex-col overflow-hidden rounded-3xl border border-zinc-200 bg-zinc-50 shadow-sm transition-[border-color,box-shadow] duration-300 dark:border-white/10 dark:bg-white/5";

/** Hover tipis: geser warna border 1px (tanpa ring tebal). */
const cardHoverBorder =
  "hover:border-zinc-400/85 dark:hover:border-white/25";

const imageHoverClass =
  "object-cover transition-transform duration-300 ease-out will-change-transform group-hover:scale-[1.04] motion-reduce:transition-none motion-reduce:group-hover:scale-100";
...
function BentoHoverSlider({
  images,
}: {
  images: readonly [HeroImage, HeroImage, HeroImage];
}) {
  return (
    <div className={`${visualWrapClass} isolate`}>
      <div className="bento-collage-track flex h-full gap-2 p-2">
        {images.map((img, i) => (
          <div
            key={`${img.src}-${i}`}
            className="relative h-full flex-1 overflow-hidden rounded-2xl bg-white/40 ring-1 ring-black/5 dark:bg-white/5 dark:ring-white/10"
          >
            <Image
              src={img.src}
              alt={img.alt}
              ...
              className={collagePanelImageClass}
              draggable={false}
            />
            {img.badge ? (
      ...
      <div className="space-y-4">
        <header className="space-y-1">
          ...
          <p className="text-base text-zinc-600 dark:text-zinc-500">
            Empat hal yang selalu aku jaga.
          </p>
        </header>
            ...
            return (
              <li
                ...
                className={[
                  ...
                  .join(" ")}
              >
                {hasSlider && c.sliderImages ? (
                  <BentoHoverSlider images={c.sliderImages} />
                ) : hasHero && c.heroImage ? (
                  <BentoSingleImage image={c.heroImage} />
                ) : null}
                <div
                  className={
                    hasVisual
