...
/**
 * Carousel foto reusable — satu-satunya komponen yang tahu cara gambar disajikan
 * (presentation layer). Awalnya milik Speaking, kini dipakai bersama Writing agar pengalaman
 * slideshow konsisten di seluruh situs. Struktur data (`GalleryImage[]`) tidak berubah;
 * berubah jadi lightbox/format lain cukup di file ini.
 *
 ...
 *
 *   public/
 *   └── <fitur>/            mis. speaking/ atau writing/
 *       └── <slug>/
 *           ├── cover.png      → cover fitur terkait
 *           ├── image-01.webp  ┐
 *           ├── image-02.webp  ├─ images (dokumentasi/gallery)
 *           └── image-03.webp  ┘
 *
...
export function MediaGallery({
  ...
  images: GalleryImage[];
  /** Label aksesibilitas untuk carousel; sesuaikan per konteks (mis. "Dokumentasi kegiatan"). */
  ariaLabel?: string;
  /** Wrapper class — sisakan `group relative` (dipakai reveal tombol) saat mengubah margin. */
  className?: string;
}) {
